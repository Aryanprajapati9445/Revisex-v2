    ...
    <div className="flex flex-col gap-24">
      <section className="grid grid-cols-1 items-center gap-10 pt-8 lg:grid-cols-[1.1fr_0.9fr] lg:gap-16">
        <div className="flex flex-col gap-6">
          <span className="text-caption font-medium text-text-muted">College notes, organized</span>
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            ...
            {isAuthenticated ? (
              <Link
                to="/upload"
                className="rounded-full bg-accent-subtle px-5 py-2.5 text-ui font-medium text-accent transition-colors duration-150"
              >
                Upload a note
              </Link>
            ) : (
        ...
        <div className="relative hidden lg:block" aria-hidden="true">
          <div className="absolute -top-4 -left-4 w-64 rounded-panel bg-background p-4 shadow-raised">
            <span className="text-caption font-medium text-text-muted">CSE</span>
            <p className="mt-1 text-base font-medium">Computer Science</p>
            <p className="mt-1 text-caption text-text-tertiary">8 semesters</p>
          </div>
          <div className="ml-16 mt-20 w-64 rounded-panel bg-background p-4 shadow-floating">
          ...
          <div className="ml-6 mt-6 w-64 rounded-card bg-surface p-4">
            <span className="inline-block rounded-full bg-status-approved-bg px-2.5 py-1 text-caption font-medium text-status-approved-fg">
              Approved
            </span>
      ...
      <section className="flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {FEATURES.map((feature) => (
            <div key={feature.title} className="rounded-card bg-background p-5 shadow-raised">
